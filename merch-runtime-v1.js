...
const merchCfg=window.DEMENTOR_SITE_CONFIG?.merch||{};

/* COMMERCE_TRUTH_GUARD_START */
function derivePublicCommerceState(rawState,commerce){
  const raw=String(rawState||'').trim().toLowerCase();
  const checkoutReady=Boolean(commerce?.checkoutEnabled&&commerce?.checkoutProvider&&commerce?.checkoutUrl);
  const preorderReady=Boolean(checkoutReady&&commerce?.preorderPaymentMethod);
  if(raw==='available'&&checkoutReady)return 'AVAILABLE';
  if(raw==='preorder'&&preorderReady)return 'PREORDER';
  // Raw source states such as sold_out are not public historical commerce evidence by themselves.
  // Until a separately approved canonical fact exists, readiness-limited public truth stays NOT OPEN.
  return 'NOT OPEN';
}
/* COMMERCE_TRUTH_GUARD_END */
...
if(cfg?.enabled&&cfg.url&&cfg.publishableKey){
  ...
  if(!error){
    ...
    const publicState=item=>item?derivePublicCommerceState(item.sales_state,merchCfg):'STATUS UNAVAILABLE';
    const checkoutReady=Boolean(merchCfg.checkoutEnabled&&merchCfg.checkoutProvider&&merchCfg.checkoutUrl);
    ...
    if(path.endsWith('/merch/')||path.endsWith('/merch/index.html')){
      ...
      if(objRow){const st=objRow.querySelector('.dc-entity-row__status');if(st)st.textContent=obj?`${money(obj.base_price_eur)} / ${publicState(obj)}`:'PRICE UNAVAILABLE / STATUS UNAVAILABLE'}
      ...
      document.querySelectorAll('.dc-drop-card').forEach(card=>{
        ...
        if(spans[1])spans[1].textContent=publicState(item);
      });

      const publicStates=[...items.values()].map(publicState);
      const open=publicStates.filter(x=>['AVAILABLE','PREORDER'].includes(x)).length;
    ...
    if(route){
      ...
      const sales=publicState(item);
      document.documentElement.dataset.dcMerchState=sales.toLowerCase().replace(/\s+/g,'_');
      fact('Price',price);
      ...
      document.querySelectorAll('[data-dc-commerce-action]').forEach(el=>{const canCheckout=Boolean(item&&checkoutReady&&['AVAILABLE','PREORDER'].includes(sales));el.hidden=!canCheckout;el.setAttribute('aria-disabled',String(!canCheckout))});
    }
