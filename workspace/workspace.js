...
const state={client:null,session:null,user:null,profile:null,membership:null,roles:[],assignments:[],entities:[],programs:new Map(),events:new Map(),tests:[],enrollments:[],orders:[],joinApplications:[],route:'home',entityId:null,systemView:false};
...
async function loadIdentity(){
  ...
  if(!session){setSessionBox();gate('ДОБРО ПОЖАЛОВАТЬ В DEMENTOR CLUB','Войдите через Google, чтобы открыть личный кабинет, историю тестов, курсов и покупок.','<button type="button" class="dcw-login" data-login>ВОЙТИ ЧЕРЕЗ GOOGLE</button>');view.querySelector('[data-login]')?.addEventListener('click',()=>login().catch(showError));return false;}
  ...
  const results=await Promise.all([qProfile,qMembership,qRoles,qAssignments,qTests,qCourses,qOrders,qJoin]);
  for(const r of results)if(r.error)throw r.error;
  state.profile=results[0].data||null;state.membership=results[1].data||null;state.roles=results[2].data||[];state.assignments=(results[3].data||[]).filter(isActiveWindow);state.tests=results[4].data||[];state.enrollments=results[5].data||[];state.orders=results[6].data||[];state.joinApplications=results[7].data||[];
