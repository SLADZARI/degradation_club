...
function show(id){['dc9Picker','dc9Play','dc9Finalizing'].forEach(view=>$(view).hidden=view!==id);window.scrollTo(0,0)}
...
function sphereDetail(si,s){const result=resultFor(si);if(result){const level=Math.max(0,Math.min(5,Math.round(Number(result.level)||0)));return `${DATA.levelNames?.[level]||'Результат зафиксирован'} · ${level}/5`}const draft=draftFor(si);if(draft){const answered=(draft.answers||[]).filter(v=>v!==null&&v!==undefined).length;return `ПРОДОЛЖИТЬ · ${answered}/6`}return s.rule}
function renderPicker(){show('dc9Picker');const done=completeCount();if(done===9){showFinalizing();return}const hasDraft=Object.keys(db.drafts||{}).some(id=>db.drafts[id]?.quizVersion===QUIZ_VERSION);$('dc9PickerKicker').textContent=done===0?'DC-9 / ВХОДНОЕ ОБСЛЕДОВАНИЕ':`DC-9 / ${done} ИЗ 9 ГОТОВО`;$('dc9JourneyBar').style.width=`${done/9*100}%`;$('dc9JourneyText').textContent=`${done} / 9`;$('dc9ResetPicker').hidden=!(done>0||hasDraft);$('dc9SphereGrid').innerHTML=DATA.spheres.map((s,i)=>`<button class="dc9-sphere${completed(i)?' is-done':''}" data-sphere="${i}" type="button"><span class="dc9-sphere-num">${s.n}</span><strong>${esc(s.title)}</strong><p>${esc(sphereDetail(i,s))}</p></button>`).join('');document.querySelectorAll('.dc9-sphere').forEach(button=>button.onclick=()=>startSphere(Number(button.dataset.sphere)))}
...
function resetAll(){if(!confirm('Удалить локальный прогресс DC-9 и начать заново?'))return;db={results:{},active:null,drafts:{},quizVersion:QUIZ_VERSION};saveDb();renderPicker()}
$('dc9ResetPicker').onclick=resetAll;$('dc9Back').onclick=renderPicker;$('dc9Next').onclick=()=>{if(chosen===null)return;if(currentQ<5){currentQ++;renderQuestion()}else finishSphere()};
const requested=canonicalSphere(new URLSearchParams(location.search).get('sphere'));const requestedIndex=SPHERE_IDS.indexOf(requested);if(requestedIndex>=0)startSphere(requestedIndex);else renderPicker();
})();
