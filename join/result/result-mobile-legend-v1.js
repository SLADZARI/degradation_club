(()=>{
  ...
  const render=()=>{
    ...
    const next=labels.map((label,i)=>`<div class="dc-radar-mobile-legend__item"><span class="dc-radar-mobile-legend__num">${String(i+1).padStart(2,'0')}</span><span class="dc-radar-mobile-legend__title">${label.textContent||''}</span><span class="dc-radar-mobile-legend__level">${levels[i]?.textContent||'—'}</span></div>`).join('');
