...
const countEl=$('resultCount'),heroStamp=$('heroStamp'),heroLead=$('heroLead'),radarStage=$('radarStage'),mapAsideTitle=$('mapAsideTitle'),mapAsideCopy=$('mapAsideCopy'),highlightGrid=$('highlightGrid'),register=$('sphereRegister'),clubNote=$('clubNote'),dossierSection=$('dossierSection'),dossierRadar=$('dossierRadar'),dossierRows=$('dossierRows'),shareNote=$('shareNote'),nextTitle=$('nextTitle'),nextCopy=$('nextCopy'),nextActions=$('nextActions'),communityState=$('communityState');
const MARKERS=['A','B','C'];
...
function marker(rank){return MARKERS[rank]||String(rank+1)}
...
function focusPoint(cx,cy,r,item,total){const actualScale=(item.level??0)/5,markerScale=Math.max(actualScale,.115);return{actual:point(cx,cy,r*actualScale,item.index,total),marker:point(cx,cy,r*markerScale,item.index,total),offset:markerScale!==actualScale}}
...
function itemFromResult(id,title,index,result){
  ...
  return{...model,title,index,result:result||null,level,levelName:level==null?'Не обследовано':DC9_LEVEL_NAMES[level]||'Уровень зафиксирован',quip:level==null?'Эта сфера пока подозрительно цела.':model.quips?.[level]||'Результат зафиксирован.',tagLevels:Array.isArray(result?.tagLevels)?result.tagLevels.slice(0,4).map(clampLevel):[],intent:clampGuard(result?.intent),responsibility:clampGuard(result?.responsibility)};
}
function qualityText(item){
  if(!item.result)return 'НЕ ОБСЛЕДОВАНО';
  if(item.intent==null||item.responsibility==null)return 'КОНТРОЛЬНЫЕ ОСИ НЕ ЗАПИСАНЫ';
  return Math.min(item.intent,item.responsibility)<2?`ИТОГ ОГРАНИЧЕН · ОСОЗНАННОСТЬ ${item.intent}/3 · ОТВЕТСТВЕННОСТЬ ${item.responsibility}/3`:`ОСОЗНАННОСТЬ ${item.intent}/3 · ОТВЕТСТВЕННОСТЬ ${item.responsibility}/3`;
}
function tagHtml(item){return item.result?`<div class="dc-tagline">${item.tags.map((tag,i)=>`<span>${esc(tag)}${item.tagLevels[i]!=null?` · ${item.tagLevels[i]}/5`:''}</span>`).join('')}</div>`:''}
...
function renderRadar(items,highlights,{dossier=false}={}){
  ...
  const labels=dossier?'':items.map((item,i)=>{const[lx,ly]=point(cx,cy,r+58,i,total),anchor=lx<cx-30?'end':lx>cx+30?'start':'middle';return `<text class="dc-radar-label" x="${lx}" y="${ly}" text-anchor="${anchor}">${esc(item.title.toUpperCase())}</text><text class="dc-radar-level" x="${lx}" y="${ly+17}" text-anchor="${anchor}">${item.level==null?'—':item.level+'/5'}</text>`}).join('');
  const focus=highlights.map((item,rank)=>{const pos=focusPoint(cx,cy,r,item,total),[ax,ay]=pos.actual,[mx,my]=pos.marker;return `<g class="dc-radar-focus">${pos.offset?`<line x1="${ax}" y1="${ay}" x2="${mx}" y2="${my}"/>`:''}<circle cx="${mx}" cy="${my}" r="17"/><text x="${mx}" y="${my+4}" text-anchor="middle">${marker(rank)}</text></g>`}).join('');
...
function renderHighlightCard(item,rank){return `<article class="dc-highlight-card"><div class="dc-highlight-top"><span class="dc-highlight-rank">${marker(rank)}</span><img class="dc-sphere-icon" src="${esc(route(item.icon))}" alt=""><strong>${esc(item.title)}</strong><b>${item.level}/5</b></div><div class="dc-level-name">${esc(item.levelName)}</div><p>${esc(item.quip)}</p>${tagHtml(item)}<div class="dc-quality">${esc(qualityText(item))}</div></article>`}
function renderSphereCard(item){return `<article class="dc-sphere-card${item.result?'':' is-missing'}"><div class="dc-sphere-card__head"><img class="dc-sphere-icon" src="${esc(route(item.icon))}" alt=""><span class="dc-sphere-card__num">${esc(item.number)}</span><strong>${esc(item.title)}</strong><b>${item.level==null?'—':item.level+'/5'}</b></div><div class="dc-level-name">${esc(item.levelName)}</div><p>${esc(item.quip)}</p>${tagHtml(item)}${item.result?`<div class="dc-quality">${esc(qualityText(item))}</div>`:''}</article>`}

function renderAll(items){
  const completed=items.filter(x=>x.result&&x.level!=null),highlights=presentationProminence(items),done=completed.length===9;
  ...
  if(highlights.length){mapAsideTitle.textContent='САМЫЕ ЗАМЕТНЫЕ МЕСТА.';mapAsideCopy.textContent=highlights.map((x,i)=>`${marker(i)} — ${x.title} ${x.level}/5`).join(' · ')+'.';highlightGrid.innerHTML=highlights.map(renderHighlightCard).join('')}else{mapAsideTitle.textContent='ПОКА СМОТРИМ.';mapAsideCopy.textContent='Пройдите хотя бы одну сферу.';highlightGrid.innerHTML=''}
  ...
  clubNote.hidden=!done;dossierSection.hidden=!done;
  if(done){dossierRadar.innerHTML=renderRadar(items,highlights,{dossier:true});dossierRows.innerHTML=highlights.map((item,rank)=>`<div class="dc-dossier-row"><span class="dc-dossier-rank">${marker(rank)}</span><img class="dc-sphere-icon" src="${esc(route(item.icon))}" alt=""><strong>${esc(item.title)}</strong><b>${item.level}/5</b><p>${esc(item.quip)}</p></div>`).join('');wireDossier(items,highlights)}
...
async function iconDataUri(path){const response=await fetch(route(path));if(!response.ok)throw new Error(`ICON_FETCH_${response.status}`);return `data:image/svg+xml;base64,${bytesToBase64(new TextEncoder().encode(await response.text()))}`}
function wrapText(text,maxChars){const words=String(text).split(/\s+/),lines=[];let line='';for(const word of words){const next=line?`${line} ${word}`:word;if(next.length>maxChars&&line){lines.push(line);line=word}else line=next}if(line)lines.push(line);return lines}
...
async function dossierSvg(items,highlights){
  ...
  const shape=items.map((item,i)=>point(cx,cy,r*((item.level??0)/5),i,total).join(',')).join(' ');
  const focus=highlights.map((item,rank)=>{const pos=focusPoint(cx,cy,r,item,total),[ax,ay]=pos.actual,[mx,my]=pos.marker;return `${pos.offset?`<line x1="${ax}" y1="${ay}" x2="${mx}" y2="${my}" stroke="#111" stroke-width="2"/>`:''}<circle cx="${mx}" cy="${my}" r="18" fill="#111"/><text x="${mx}" y="${my+5}" text-anchor="middle" font-family="Arial" font-size="12" font-weight="900" fill="#f2f0e8">${marker(rank)}</text>`}).join('');
  const icons=await Promise.all(highlights.map(x=>iconDataUri(x.icon)));
  const rows=highlights.map((item,rank)=>{const y=910+rank*112,lines=wrapText(item.quip,43).slice(0,2),tspans=lines.map((line,i)=>`<tspan x="455" dy="${i?30:0}">${esc(line)}</tspan>`).join('');return `<line x1="65" y1="${y-48}" x2="1015" y2="${y-48}" stroke="#111" stroke-opacity=".22"/><circle cx="83" cy="${y-7}" r="15" fill="#111"/><text x="83" y="${y-3}" text-anchor="middle" font-family="Arial" font-size="11" font-weight="900" fill="#f2f0e8">${marker(rank)}</text><image href="${icons[rank]}" x="112" y="${y-28}" width="38" height="38"/><text x="170" y="${y}" font-family="Arial" font-size="29" font-weight="900" fill="#111">${esc(item.title.toUpperCase())}</text><text x="355" y="${y}" font-family="Arial" font-size="29" font-weight="900" fill="#111">${item.level}/5</text><text x="455" y="${y}" font-family="Arial" font-size="24" font-weight="800" fill="#111">${tspans}</text>`}).join('');
...
function wireDossier(items,highlights){
  const download=$('downloadDossier'),share=$('shareDossier');if(!download||!share)return;const make=async()=>svgToPngFile(await dossierSvg(items,highlights));
  download.onclick=async()=>{download.disabled=true;shareNote.textContent='СОБИРАЕМ ДОСЬЕ…';try{const file=await make(),url=URL.createObjectURL(file),a=document.createElement('a');a.href=url;a.download=file.name;a.click();setTimeout(()=>URL.revokeObjectURL(url),1200);shareNote.textContent='ДОСЬЕ ГОТОВО.'}catch(error){shareNote.textContent='НЕ УДАЛОСЬ СОБРАТЬ ДОСЬЕ.';console.error(error)}finally{download.disabled=false}};
...
async function boot(){
  ...
  const results=await readSphereResults(client,session?.user?.id||null),items=DC_SPHERES.map(([id,title],index)=>itemFromResult(id,title,index,results[id])),state=renderAll(items);
