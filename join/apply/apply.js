...
if(!host||!cfg?.enabled){
  if(host)host.innerHTML='<div class="dc-apply-error">Сервис заявок сейчас недоступен.</div>';
}else{
  ...
  if(!user){
    ...
    host.querySelector('[data-login]').onclick=login;
  }else{
    const uid=user.id;
    ...
    if(member){
      host.innerHTML='<section class="dc-apply-done"><div class="dc-apply-kicker">MEMBERSHIP / ACTIVE</div><h2>ВЫ УЖЕ<br>В КЛУБЕ.</h2><p>Повторная заявка не требуется.</p><p><a class="dc-apply-submit" href="'+base+'/workspace/">ОТКРЫТЬ ЛИЧНЫЙ КАБИНЕТ →</a></p></section>';
      ...
      const body=current.status==='accepted'
        ?'Два дементора подтвердили заявку. Членство активируется сервером.'
        :'Заявку рассматривают дементоры. Один голос сам по себе членство не создаёт.';
      ...
      host.innerHTML=`<section class="dc-apply-done"><div class="dc-apply-kicker">APPLICATION / CONTINUE</div><h2>ПРОДОЛЖАЕМ<br>СНАРУЖИ.</h2><p>Community сейчас не активирована. Ваша Sphere Map и аккаунт остаются с вами — это не отменяет результаты обследования.</p><p><a class="dc-apply-submit" href="${base}/">В DEMENTOR CLUB →</a></p></section>`;
    }else if(!gateComplete){
      host.innerHTML=`<section class="dc-apply-gate"><div class="dc-apply-kicker">MEMBERSHIP GATE / ${sphereCount} OF 9</div><h2>СНАЧАЛА<br>ДЕВЯТЬ.</h2><p>Заявка в Community появляется только после всех девяти сфер. Сейчас сервер видит ${sphereCount}/9.</p><p><a class="dc-apply-submit" href="${base}/join/">ПРОДОЛЖИТЬ DC-9 →</a></p></section>`;
    }else{
      renderForm(profile);
    }
    ...
    function renderForm(profile){
      ...
      host.innerHTML=`<form class="dc-apply-form" id="membershipForm">
        ...
        <section class="dc-apply-foot">
          <label class="dc-field is-wide"><span>Перед отправкой</span><span><input type="checkbox" name="legal_accepted" required> Подтверждаю <a href="${base}/legal/terms/" target="_blank" rel="noopener">условия клуба</a> и <a href="${base}/legal/privacy/" target="_blank" rel="noopener">политику приватности</a>.</span></label>
