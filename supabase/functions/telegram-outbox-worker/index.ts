...
Deno.serve(async (req: Request) => {
  ...
  if (queueError) return json({ error: "Outbox unavailable", detail: queueError.message }, 500);
  ...
  for (const row of queue || []) {
    ...
    try {
      ...
      if (media?.storage_path && media.media_type === "image") {
        const { data: signed } = await admin.storage.from("dc-community-artifacts").createSignedUrl(media.storage_path, 600);
        if (signed?.signedUrl) {
