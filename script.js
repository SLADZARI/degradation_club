...
(()=>{
  ...
  function decorateCards(){[...grid.querySelectorAll('.sphere')].forEach(card=>{const title=card.querySelector('strong')?.textContent?.trim();if(!title||!cardCopy[title])return;const body=card.querySelector('p');if(body&&body.dataset.procedureCopy!==title){body.innerHTML=cardCopy[title];body.dataset.procedureCopy=title}const done=!!card.querySelector('.badge.done');const foot=card.querySelector('.sphere-foot');if(!foot)return;foot.querySelectorAll(':scope > .badge').forEach(x=>x.remove());let ui=foot.querySelector(':scope > [data-procedure-ui]');if(!ui){ui=document.createElement('span');ui.className='procedure-ui';ui.dataset.procedureUi='1';foot.prepend(ui)}const stateKey=done?'done':'new';if(ui.dataset.state!==stateKey){ui.dataset.state=stateKey;ui.innerHTML=`<span class="procedure-status">${done?'УХУДШЕНИЕ ПОДТВЕРЖДЕНО':'ДЕГРАДАЦИЯ НЕ НАЧАТА'}</span><span class="procedure-cta">${done?'УХУДШИТЬ ЕЩЁ РАЗ →':'НАЧАТЬ ДЕГРАДАЦИЮ →'}</span>`}})}
