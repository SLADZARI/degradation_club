...
function guestNotice(row,index){
  ...
  return `<article class="dc-notice dc-notice--guest${historical?' is-history':''}" data-artifact="${esc(row.artifact_id)}" data-artifact-status="${esc(row.status||'active')}" data-guest-read="1" data-member-reaction-count="${Math.max(0,Number(row.reaction_count||0))}">
