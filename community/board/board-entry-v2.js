...
function personalCardMarkup(state){
  ...
  if(state.key===BOARD_USER_STATES.AUTHENTICATED_GUEST_DC9_INCOMPLETE){
    return `<article class="dc-personal-card" data-personal-state="guest-dc9-incomplete" tabindex="-1"><div class="dc-personal-card__meta"><span>МОЙ СТАТУС / GUEST</span><span>DC-9 ${count} / 9</span></div><h3>ВСТУПИТЬ<br>В КЛУБ.</h3><p>Доску можно смотреть уже сейчас. Для заявки нужен первый полный проход DC-9.</p><a class="dc-personal-card__action" href="${route('/join/')}">ПРОЙТИ / ПРОДОЛЖИТЬ DC-9 →</a></article>`;
  }
  if(state.key===BOARD_USER_STATES.AUTHENTICATED_GUEST_DC9_COMPLETE){
    return `<article class="dc-personal-card" data-personal-state="guest-dc9-complete" tabindex="-1"><div class="dc-personal-card__meta"><span>МОЙ СТАТУС / READY</span><span>DC-9 9 / 9</span></div><h3>МОЖНО<br>ПОДАВАТЬ.</h3><p>Первый полный DC-9 сохранён как membership baseline. Повторный проход его не заменит.</p><div class="dc-personal-card__actions"><a class="dc-personal-card__action" href="${route('/join/apply/')}">ПОДАТЬ ЗАЯВКУ →</a><a class="dc-personal-card__secondary" href="${route('/join/')}">ПЕРЕПРОЙТИ DC-9</a></div></article>`;
  }
  if(state.key===BOARD_USER_STATES.APPLICANT){
    const status=String(state.application?.status||'reviewing').toUpperCase();
    return `<article class="dc-personal-card" data-personal-state="applicant" tabindex="-1"><div class="dc-personal-card__meta"><span>МОЙ СТАТУС / APPLICATION</span><span>${esc(status)}</span></div><h3>ЗАЯВКА<br>В РАБОТЕ.</h3><p>Вы уже сделали следующий шаг. Членство появится только после канонического review.</p><a class="dc-personal-card__action" href="${route('/join/apply/')}">СТАТУС ЗАЯВКИ →</a></article>`;
  }
  if(isBoardMemberState(state.key)){
    ...
    return `<article class="dc-personal-card is-member" data-personal-state="member" tabindex="-1"><div class="dc-personal-card__meta"><span>МОЯ КАРТА / ${role}</span><span>${activation}</span></div><h3>МОЯ<br>КАРТА.</h3><p>${state.key===BOARD_USER_STATES.MEMBER_NOT_ACTIVATED?'Членство активно. Осмотритесь и приколите первую свою вещь, чтобы открыть полное участие на доске.':'Членство активно. Ваши Board-действия определяются текущей ролью и activation state.'}</p><a class="dc-personal-card__action" href="${route('/workspace/#club')}">УЧАСТИЕ →</a></article>`;
  }
  return '';
}
...
async function toggleGuestInterest(button){
  ...
  try{const {data,error}=await client.rpc('dc_guest_board_interest_toggle_v1',{p_artifact_id:artifactId});if(error)throw error;const result=Array.isArray(data)?data[0]:data;const active=result?.active===true;const guestCount=Math.max(0,Number(result?.count||0));const base=Math.max(0,Number(card.dataset.memberReactionCount||0));button.classList.toggle('active',active);button.setAttribute('aria-pressed',active?'true':'false');const label=button.querySelector('[data-guest-interest-label]');if(label)label.textContent=`${active?'✓':'☆'} ИНТЕРЕСНО ·`;const countEl=button.querySelector('[data-guest-interest-count]');if(countEl)countEl.textContent=String(base+guestCount)}catch(error){const message=document.createElement('div');message.className='dc-board-error';message.setAttribute('role','status');message.textContent=errorMessage(error);card?.appendChild(message);setTimeout(()=>message.remove(),5000)}finally{button.disabled=false;guestInterestBusy=false}
}
...
async function boot(){const state=await resolveBoardUserState(client);document.documentElement.dataset.dcBoardUserState=state.key;if(state.key===BOARD_USER_STATES.UNAUTHENTICATED){await import('./board.js');return}publishPersonalCard(state);if(isBoardMemberState(state.key)){if(entrySection)entrySection.hidden=false;await import('./board.js');return}await renderGuestBoard(state)}
boot().catch(error=>{boardStatus.textContent='ERROR';fail(error,entryHost);fail(error,boardHost)});
