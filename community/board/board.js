...
function renderComposer(draft){
  ...
  entryHost.innerHTML=`<div class="dc-composer"><div class="dc-composer-head"><span>ARTIFACT / NOTICE<br>${draft?'DRAFT / SAVED':'SLOT / READY'}</span><div><h2>ЕСЛИ БЫ ВЫ БЫЛИ ДЕМЕНТОРОМ —<br>ЧТО БЫ ВЫ ПРЕДЛОЖИЛИ ДРУГИМ?</h2></div></div><form class="dc-composer-form" id="artifactForm">
    ...
    <div class="dc-composer-field"><label for="artifactUrl">Ссылка</label><input id="artifactUrl" name="external_url" maxlength="1000" inputmode="url" value="${esc(draft?.external_url||'')}" placeholder="https://…"><small>Опциональная внешняя ссылка.</small></div>
    <div class="dc-composer-field"><label for="artifactExpires">Срок действия</label><input id="artifactExpires" name="expires_at" type="datetime-local" value="${esc(localDateInput(draft?.expires_at))}"><small>Оставьте пустым для постоянного объявления. Разовая встреча или событие должны иметь срок.</small></div>
    <div class="dc-composer-field"><label for="artifactFile">Изображение / файл</label><div><input id="artifactFile" name="media" type="file" accept="image/jpeg,image/png,image/webp,application/pdf,text/plain" ${media?'disabled':''}>${media?`<div class="dc-board-state">УЖЕ ПРИКРЕПЛЕНО: ${esc(media.metadata?.name||media.storage_path.split('/').pop())}</div>`:''}</div><small>Первая версия: один файл, максимум 8 MB. JPG / PNG / WebP / PDF / TXT. Файл хранится в закрытом Community bucket.</small></div>
...
async function publishFromForm(event){
  event.preventDefault();const form=event.currentTarget;const submit=form.querySelector('button[type=submit]');const state=document.getElementById('composerState');const fd=new FormData(form);
  const body=String(fd.get('body')||'').trim();const title=String(fd.get('title')||'').trim()||null;const externalUrl=String(fd.get('external_url')||'').trim()||null;const expiresAt=iso(String(fd.get('expires_at')||''));const file=form.querySelector('#artifactFile')?.files?.[0]||null;
  if(!body){boardError('Введите текст объявления.');return}
  if(file&&(!allowedTypes.has(file.type)||file.size>maxFileSize)){boardError(file.size>maxFileSize?'Файл больше 8 MB.':'Этот тип файла пока не поддерживается.');return}
  ...
  try{
    ...
    if(file&&!ownDraftMedia.length){
      state.textContent='ЗАГРУЖАЕМ ФАЙЛ';const path=`${session.user.id}/${artifactId}/${Date.now()}-${safeFileName(file.name)}`;const uploaded=await client.storage.from(DC_ARTIFACT_BUCKET).upload(path,file,{upsert:false,contentType:file.type});if(uploaded.error)throw uploaded.error;uploadedPath=path;
      const attached=await client.rpc('dc_attach_artifact_media_v1',{p_artifact_id:artifactId,p_storage_path:path,p_media_type:mediaType(file),p_metadata:{name:file.name,size:file.size,mime:file.type}});if(attached.error){await client.storage.from(DC_ARTIFACT_BUCKET).remove([path]).catch(()=>{});throw attached.error}
    }
  ...
  }catch(error){submit.disabled=false;submit.textContent='ОПУБЛИКОВАТЬ →';state.textContent='НЕ ОПУБЛИКОВАНО';if(uploadedPath)console.warn('[DC Board] uploaded object retained/cleanup attempted',uploadedPath);boardError(error)}
}
...
async function removeDraft(){
  ...
  }catch(error){if(button){button.disabled=false;button.textContent='УДАЛИТЬ ЧЕРНОВИК'}boardError(error)}
}
...
async function loadBoard(){
  ...
  if(!artifacts.length){boardHost.innerHTML='<div class="dc-board-empty"><h3>ДОСКА ПОКА<br>ПОДОЗРИТЕЛЬНО ЧИСТАЯ.</h3><p>Исторические клубные материалы будут добавляться только из подтверждённых источников. Первый живой Artifact может появиться прямо сейчас.</p></div>';return}
