...
function promotionControls(){
  ...
  if(isOwnerAdmin()){
    if(status==='held')parts.push('<button class="dc-artifact-action" type="button" id="detailAdminPromote">ОПУБЛИКОВАТЬ В TELEGRAM</button>');
...
function render(profile,media){
  const mine=!guestMode&&artifact.author_profile_id===session.user.id;const myReaction=guestMode?guestInterest:reactions.some(r=>r.profile_id===session.user.id);const myResponse=guestMode?(guestResponseSubmitted?{status:'submitted'}:null):responses.find(r=>r.responder_profile_id===session.user.id&&r.status==='submitted');const incoming=mine?responses.filter(r=>r.status==='submitted').length:0;const historical=isHistorical();
  ...
  document.getElementById('detailReaction').onclick=toggleReaction;document.getElementById('detailClose')?.addEventListener('click',closeArtifact);document.getElementById('detailResponse')?.addEventListener('click',openResponse);document.getElementById('detailPromotionSupport')?.addEventListener('click',supportPromotion);document.getElementById('detailAdminPromote')?.addEventListener('click',adminPromote);document.getElementById('detailAdminSuppress')?.addEventListener('click',adminSuppress);document.getElementById('detailAdminHide')?.addEventListener('click',adminHide);document.getElementById('detailResolveSent')?.addEventListener('click',()=>resolveUnknown('sent'));document.getElementById('detailResolveRetry')?.addEventListener('click',()=>resolveUnknown('retry'));document.getElementById('detailResolveCancel')?.addEventListener('click',()=>resolveUnknown('cancelled'));bindBoardReturn();
}
...
async function adminPromote(){const result=await client.rpc('dc_admin_promote_artifact_telegram_v1',{p_artifact_id:artifact.id});if(result.error){fail(result.error);return}await load()}
