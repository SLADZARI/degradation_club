(()=>{
  ...
  const objectProduct={sku:'DC-OBJECT-001',title:'OBJECT 001 — НЕ НАДО',price:520,hasSizes:false,variants:[]};
  ...
  mounts.forEach((mount,index)=>{
    ...
    const pageProduct={sku:mount.dataset.sku||'SKU',title:mount.dataset.title||mount.dataset.sku||'PRODUCT',price:Number(mount.dataset.price||0),hasSizes:mount.dataset.sizes!=='false',variants};
    ...
    const renderOrder=(product)=>{
      ...
      shell.innerHTML=`<section class="dc-order-form"><div class="dc-order-form__head"><span>PREORDER / FORM</span><div><h2>СНАЧАЛА<br>ЗАФИКСИРУЕМ.</h2><p>Выберите позицию и оставьте минимум данных для ручного подтверждения. BLIK показывается только на следующем шаге.</p></div></div><div class="dc-order-form__grid"><form class="dc-order-form__fields" novalidate>${product.variants.length?`<label>Variant<select name="variant">${product.variants.map(v=>`<option value="${esc(v.label)}" data-price="${v.price}">${esc(v.label)} — €${v.price}</option>`).join('')}</select></label>`:''}${product.hasSizes?`<fieldset><legend>Size</legend><div class="dc-order-sizes">${sizes.map(s=>`<button type="button" data-size="${s}" class="${s==='M'?'is-active':''}">${s}</button>`).join('')}</div></fieldset>`:''}<label>Имя<input name="name" autocomplete="name" maxlength="120" required placeholder="Как к вам обращаться"></label><label>Email или Telegram<input name="contact" maxlength="200" required placeholder="@telegram или email"></label><label>Страна доставки<select name="country"><option>Poland</option><option>Germany</option><option>Czech Republic</option><option>Lithuania</option><option>Latvia</option><option>Estonia</option><option>France</option><option>Spain</option><option>Italy</option><option>Netherlands</option><option>Other EU</option></select></label><label>Город<input name="city" maxlength="120" required placeholder="Warsaw"></label><label class="dc-order-form__full">Комментарий — необязательно<textarea name="note" maxlength="1000" placeholder="Вопрос по размеру, подарок, удобный канал связи"></textarea></label><button type="submit" class="dc-order-submit"><span>ПРОДОЛЖИТЬ К ОПЛАТЕ</span><span>→</span></button><p class="dc-order-form__legal">Заказ считается подтверждённым после поступления BLIK и ручной сверки.</p></form><aside class="dc-order-summary" aria-live="polite"><span class="dc-order-summary__label">ORDER PREVIEW</span><h3>${esc(product.title)}</h3><div><span>SKU</span><strong>${esc(product.sku)}</strong></div>${product.variants.length?`<div><span>Variant</span><strong data-summary-variant>${esc(defaultVariant)}</strong></div>`:''}${product.hasSizes?'<div><span>Size</span><strong data-summary-size>M</strong></div>':''}<div><span>State</span><strong>PREORDER</strong></div><div class="dc-order-summary__total"><span>TOTAL</span><strong data-summary-price>€${defaultPrice}</strong></div><div class="dc-order-payment" hidden><span>PREORDER / NEXT STEP</span><strong>BLIK → +48 573 265 211</strong><p>Укажите в сообщении к переводу:</p><code data-payment-ref></code><p>После поступления платежа заказ подтверждается вручную по указанному контакту.</p></div></aside></div></section>`;
      const form=shell.querySelector('form');const payment=shell.querySelector('.dc-order-payment');let size='M';
      ...
      const variant=form.elements.variant;if(variant)variant.addEventListener('change',()=>{const opt=variant.selectedOptions[0];shell.querySelector('[data-summary-variant]').textContent=variant.value;shell.querySelector('[data-summary-price]').textContent=`€${opt.dataset.price}`;payment.hidden=true;});
      form.addEventListener('submit',e=>{e.preventDefault();const name=form.elements.name.value.trim(),contact=form.elements.contact.value.trim(),city=form.elements.city.value.trim();if(!name||!contact||!city){form.reportValidity();return;}const variantValue=variant?variant.value:'';shell.querySelector('[data-payment-ref]').textContent=[product.sku,variantValue,product.hasSizes?size:'',refId()].filter(Boolean).join(' / ');payment.hidden=false;payment.scrollIntoView({behavior:'smooth',block:'nearest'});});
    };
