(()=>{
  ...
  const objectImages=[
    '/assets/objects/object-001/object-001-hero-3q.webp',
    '/assets/objects/object-001/object-001-top.webp',
    '/assets/objects/object-001/object-001-back.webp',
    '/assets/objects/object-001/object-001-macro-front.webp',
    '/assets/objects/object-001/object-001-packaging.webp'
  ];
  ...
  mounts.forEach((mount,index)=>{
    ...
    const pageProduct={
      sku:mount.dataset.sku||'SKU',
      title:mount.dataset.title||mount.dataset.sku||'PRODUCT',
      price:Number(mount.dataset.price||0),
      hasSizes:mount.dataset.sizes!=='false',
      variants
    };

    mount.innerHTML=`<div class="dc-commerce-actions">
      <button class="dc-commerce-action dc-commerce-action--primary" type="button" data-open-preorder>PREORDER <span>→</span></button>
    ...
    if(pageProduct.sku!=='DC-OBJECT-001'){
      ...
      objectModal.querySelector('[data-object-preorder]').addEventListener('click',()=>{objectModal.classList.remove('is-open');renderOrder(objectProduct);preorder.classList.add('is-open');});
    }
    ...
    const renderOrder=(product)=>{
      ...
      shell.innerHTML=`<section class="dc-order-form">
        <div class="dc-order-form__head"><span>PREORDER / FORM</span><div><h2>СНАЧАЛА<br>ЗАФИКСИРУЕМ.</h2><p>Выберите позицию и оставьте минимум данных для ручного подтверждения. BLIK показывается только на следующем шаге.</p></div></div>
        <div class="dc-order-form__grid">
          <form class="dc-order-form__fields" novalidate>
            ${product.variants.length?`<label>Variant<select name="variant">${product.variants.map(v=>`<option value="${esc(v.label)}" data-price="${v.price}">${esc(v.label)} — €${v.price}</option>`).join('')}</select></label>`:''}
            ${product.hasSizes?`<fieldset><legend>Size</legend><div class="dc-order-sizes">${sizes.map(s=>`<button type="button" data-size="${s}" class="${s==='M'?'is-active':''}">${s}</button>`).join('')}</div></fieldset>`:''}
    ...
    mount.querySelector('[data-open-preorder]').addEventListener('click',()=>{renderOrder(pageProduct);preorder.classList.add('is-open');lock();});
    const objectButton=mount.querySelector('[data-open-object]');
    if(objectButton&&objectModal)objectButton.addEventListener('click',()=>{objectModal.classList.add('is-open');lock();});
