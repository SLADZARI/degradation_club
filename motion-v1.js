(()=>{
  ...
  if(ink.target){
    ...
    if(inkTarget){
      ...
      if(ink.level===1){
        inkTarget.classList.add('dc-ink-trace');
        inkTarget.dataset.inkLabel=ink.label||'INK / L1 / TRACE';
        if(ink.media){
          const sourceRow=document.querySelector('.dc-catalog-row[data-preview-src]');
          if(sourceRow?.dataset.previewSrc){
            const fragment=document.createElement('span');
            fragment.className='dc-ink-trace-media';
            fragment.setAttribute('aria-hidden','true');
            const image=document.createElement('img');
            image.src=sourceRow.dataset.previewSrc;
            image.alt='';
            image.loading='lazy';
            image.decoding='async';
            fragment.appendChild(image);
            inkTarget.appendChild(fragment);
          }
        }
      }
    }
  ...
  document.querySelectorAll('.nav a').forEach(a=>{
    ...
    const target=new URL(href,location.origin).pathname;if(target==='/'?path==='/' : path.startsWith(target))a.classList.add('active');
  });
  ...
  /* One route -> one stable variation. */
  let seed=0;for(const ch of path)seed=(seed*31+ch.charCodeAt(0))>>>0;
  const variantRoot=document.querySelector('main');
  if(variantRoot){
    variantRoot.classList.add('dc-dia-variant');
    const tilt=(((seed%9)-4)*.035).toFixed(3),shift=((seed%7)-3)*2,inkTilt=(((seed%11)-5)*.055).toFixed(3),previewY=((seed%9)-4)*5,captionShift=((seed%5)-2)*4;
    variantRoot.style.setProperty('--dc-dia-tilt',`${tilt}deg`);variantRoot.style.setProperty('--dc-dia-shift',`${shift}px`);variantRoot.style.setProperty('--dc-dia-ink-tilt',`${inkTilt}deg`);variantRoot.style.setProperty('--dc-dia-preview-y',`${previewY}px`);variantRoot.style.setProperty('--dc-dia-caption-shift',`${captionShift}px`);
    document.documentElement.dataset.diaVariant=String(seed%4);
  }
