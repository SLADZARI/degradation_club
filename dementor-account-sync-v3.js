// Dementor Club — account + diagnostic sync v3.
// Single Supabase client, explicit PKCE code exchange, local-first onboarding.
(()=>{
  if(!location.pathname.includes('/join'))return;
  if(window.__DC_ACCOUNT_SYNC_V3__)return;
  window.__DC_ACCOUNT_SYNC_V3__=true;
  ...
  const fail=e=>{console.error('[Dementor Sync v3]',e);setStatus(`ОШИБКА СИНХРОНИЗАЦИИ${e?.message?' · '+e.message:''}`)};
  ...
  function installStorageTap(){const original=Storage.prototype.setItem;if(original.__dcSyncV3Wrapped)return;const wrapped=function(key,value){const r=original.apply(this,arguments);if(this===localStorage&&key===STORAGE&&!applyingRemote){const prev=clone(lastState);let next=null;try{next=JSON.parse(value)}catch(_){}if(next){lastState=clone(next);if(session)queueSync(250,false);const a=prev?.active,res=a?.sphere?next?.results?.[a.sphere]:null;if(a?.sphere&&!next.active&&res?.date)persistRun(a.sphere,res,a).catch(fail)}}return r};wrapped.__dcSyncV3Wrapped=true;Storage.prototype.setItem=wrapped}

  async function boot(){
    render();
    const cfg=await waitConfig();
    const mod=await import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2.112.4/+esm');
    client=mod.createClient(cfg.url,cfg.publishableKey,{auth:{persistSession:true,detectSessionInUrl:false,autoRefreshToken:true,flowType:'pkce'}});
    ...
    if(code){
      ...
      if(!session)throw new Error('OAuth code exchanged without session');
