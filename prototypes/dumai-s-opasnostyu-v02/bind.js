...
function render(){const idx=Math.max(0,screensOrder.indexOf(state.screen));document.getElementById('bar').style.width=(idx/(screensOrder.length-1))*100+'%';document.getElementById('topStep').textContent=state.currentDay?`ДЕЛО / ДЕНЬ ${state.currentDay}`:'PROTOTYPE V0.2';(screens[state.screen]||screens.landing)();bind()}
render();
