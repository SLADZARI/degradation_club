...
if(cfg?.enabled&&cfg.url&&cfg.publishableKey&&location.pathname.includes('/join')&&!location.pathname.includes('/join/apply')){
  ...
  function resultMetrics(r){const j=r?.result_json||{};const tags=Array.isArray(j.tagLevels)?j.tagLevels:[];return [
    ['Осознанность отказа',j.intent],['Ответственность за последствия',j.responsibility],...tags.map((v,i)=>[`Признак ${String(i+1).padStart(2,'0')}`,v])
  ].filter(x=>x[1]!=null)}
  ...
  function ensureResultSavePrompt(){const result=document.getElementById('result');if(!result)return;let box=result.querySelector('.dc9-save-result');if(user){box?.remove();return}if(!result.classList.contains('active')){box?.remove();return}if(box)return;box=document.createElement('div');box.className='dc9-save-result';box.innerHTML='<div><strong>РЕЗУЛЬТАТ ГОТОВ.</strong><p>Он сохранён на этом устройстве. Войдите, чтобы привязать результат к профилю и восстановить его на другом устройстве.</p></div><button type="button">СОХРАНИТЬ В ПРОФИЛЬ</button>';box.querySelector('button').onclick=loginAndSave;(result.querySelector('.actions')||result).after(box)}
  function decorate(){
    mergeLatest();const grid=document.getElementById('sphereGrid');if(!grid)return;const cards=[...grid.querySelectorAll('.sphere')];if(!cards.length)return;const done=[...latest.entries()].map(([id,r])=>({id,r,level:levelOf(r)})).filter(x=>x.level!=null);const avg=done.length?done.reduce((s,x)=>s+x.level,0)/done.length:null;const nextId=ORDER.find(id=>!latest.has(id))||ORDER.slice().sort((a,b)=>(levelOf(latest.get(a))??9)-(levelOf(latest.get(b))??9))[0];const key=JSON.stringify([done.map(x=>[x.id,x.level,when(x.r)]),[...certs],cards.length,result?.classList?.contains('active')]);if(key===lastRenderKey){ensureResultSavePrompt();return}lastRenderKey=key;
  ...
  const result=document.getElementById('result');if(result)new MutationObserver(()=>{lastRenderKey='';decorate()}).observe(result,{attributes:true,attributeFilter:['class']});
}
