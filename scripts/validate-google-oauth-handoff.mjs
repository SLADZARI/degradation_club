...
// This is an external-provider release probe. A single Supabase edge 5xx must not
// turn an unrelated candidate into a false regression, but repeated failure still
// blocks the release gate. Redirect semantics are never relaxed.
let response=null,lastNetworkError=null;
for(let attempt=1;attempt<=3;attempt++){
  try{
    response=await fetch(authorize,{redirect:'manual',headers:{'user-agent':'DementorClub-ReleaseGate/1.0'}});
...
if(!response){
  errors.push(`Supabase /authorize request failed after 3 attempts: ${lastNetworkError instanceof Error?lastNetworkError.message:String(lastNetworkError||'unknown network error')}`);
}else{
  const location=response.headers.get('location');
  expect([301,302,303,307,308].includes(response.status),`Supabase /authorize did not redirect after retries: HTTP ${response.status}`);
...
console.log('Google OAuth provider handoff PASS: built Google login owners request explicit account choice and Supabase redirects to accounts.google.com with canonical callback (transient 5xx retried up to 3 attempts)');
