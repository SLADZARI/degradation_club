...
for(const viewport of [{width:390,height:500,label:'390'},{width:430,height:844,label:'430'},{width:800,height:900,label:'800'},{width:1440,height:900,label:'desktop'}]){
  ...
  if(viewport.width<=768){const clearance=await page.evaluate(()=>{const drawer=document.querySelector('.dc-board-filter-drawer')?.getBoundingClientRect(),controls=document.querySelector('.dc-spatial-controls')?.getBoundingClientRect();return{drawerBottom:drawer?.bottom??null,controlsTop:controls?.top??null}});expect(clearance.drawerBottom!==null&&clearance.controlsTop!==null&&clearance.drawerBottom<=clearance.controlsTop-6,`drawer-${viewport.label}: mobile drawer overlaps bottom controls ${JSON.stringify(clearance)}`)}
...
for(const viewport of [{width:390,height:844,label:'390'},{width:1440,height:900,label:'desktop'}]){
  ...
  if(hit.ok){await page.mouse.click(hit.x,hit.y);const overlay=page.locator('.dc-artifact-overlay');await overlay.waitFor({state:'visible',timeout:2500});const frame=page.frameLocator('.dc-artifact-overlay iframe');await frame.locator('#artifactBackTop').waitFor({state:'visible',timeout:3500});await frame.locator('#artifactBackTop').click();await overlay.waitFor({state:'hidden',timeout:2500});expect(new URL(page.url()).pathname.replace(/\/+$/,'/')==='/workspace/board/',`member-first-${viewport.label}: parent Board route changed after embedded return ${page.url()}`);expect(await page.locator('.dc-artifact-overlay iframe').getAttribute('src')==='about:blank',`member-first-${viewport.label}: embedded return left nested Board/detail iframe alive`)}
...
console.log('Board live corrective browser QA PASS: mobile type drawer clearance + black-on-lime active filter + rendered control geometry + interactive MEMBER_NOT_ACTIVATED onboarding + embedded Artifact return');