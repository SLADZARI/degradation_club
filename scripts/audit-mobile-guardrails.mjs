...
function walk(dir){
  for(const entry of fs.readdirSync(dir,{withFileTypes:true})){
    ...
    else if(/\.(html|css|js|mjs|md|yml|yaml)$/.test(entry.name) && !p.includes('scripts/audit-mobile-guardrails.mjs') && !p.includes('scripts/batch4d2-mobile-guardrails.mjs')){
      const text=fs.readFileSync(p,'utf8');
      if(text.includes('mobile-qa.css')||text.includes('mobile-overflow-fix.css')) errors.push(`${p}: legacy mobile layer reference remains`);
    }
  }
}
walk('.');
...
console.log('Mobile guardrail audit passed: one structural layer / zero legacy files / zero !important');
