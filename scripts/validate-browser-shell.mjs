...
async function workspaceShell(page,label){
  try{await page.locator('[data-workspace-sidebar]').waitFor({state:'visible',timeout:4000})}catch{errors.push(`${label}: canonical Workspace shell did not render`);return}
  expect(await page.locator('.dc-global-header').count()===0,`${label}: public GlobalHeader leaked into Workspace`);
  expect(await page.locator('.dcw-brand').count()===1,`${label}: Workspace brand missing`);
  expect((await page.locator('.dcw-brand').getAttribute('href'))==='/',`${label}: Workspace brand does not return to public /`);
}
...

{
  ...
  if(identityReady){expect((await p.locator('[data-global-identity]').getAttribute('href'))==='/workspace/','authenticated guest: identity does not route to Workspace');expect((await p.locator('[data-global-identity]').innerText()).includes('QA Browser'),'authenticated guest: identity name missing');expect(await p.locator('[data-global-join-cta]:visible').count()===1,'authenticated guest: club-entry CTA must remain visible before membership');expect(await p.locator('[data-global-login]:visible').count()===0,'authenticated guest: login service survived after session')}
...

{
  ...
  expect(await p.locator('.dc-global-brand:visible').count()===1,'mobile guest: brand missing');expect(await p.locator('[data-global-join-cta]:visible').count()===1,'mobile guest: persistent club-entry CTA missing while menu closed');expect(await p.locator('.dc-global-menu:visible').count()===1,'mobile guest: burger missing');expect(await p.locator('#dc-global-nav:visible').count()===0,'mobile guest: burger panel is open by default');
  const geometry=await p.locator('.dc-global-header').evaluate(el=>({scrollWidth:el.scrollWidth,clientWidth:el.clientWidth}));expect(geometry.scrollWidth<=geometry.clientWidth+1,`mobile guest: header overflows at 360px ${JSON.stringify(geometry)}`);await p.locator('.dc-global-menu').click();try{await p.locator('#dc-global-nav').waitFor({state:'visible',timeout:2000})}catch{errors.push('mobile guest: burger did not open')}expect(await p.locator('#dc-global-nav [data-global-login]:visible').count()===1,'mobile guest: login is not inside opened burger');expect(!pageErrors.length,`mobile guest header errors: ${pageErrors.join(' | ')}`);await c.close();
}

// Guest Workspace: private shell brand remains as the only public escape, private nav stays hidden.
{
  const c=await context('guest'),p=await c.newPage(),pageErrors=[];p.on('pageerror',e=>pageErrors.push(e.message));
  await p.goto(base+'/workspace/',{waitUntil:'domcontentloaded'});await workspaceShell(p,'/workspace/ guest');
  ...
  expect(await p.locator('[data-workspace-nav]:visible').count()===0,'/workspace/: guest can see private navigation');expect(await p.locator('[data-member-tool]:visible').count()===0,'/workspace/: guest can see member routes');expect(await p.locator('.dc-global-header').count()===0,'/workspace/: public header survived inside private system');expect(!pageErrors.length,`/workspace/ guest errors: ${pageErrors.join(' | ')}`);await c.close();
}

// PKCE success lands at Workspace and an ordinary Member immediately resolves to Community Board.
{
  ...
  try{await p.waitForURL(u=>new URL(u).pathname==='/workspace/board/',{timeout:6000})}catch{errors.push(`/auth/callback/: ordinary Member did not resolve to Board; actual=${p.url()}`)}
  const finalPath=new URL(p.url()).pathname;expect(finalPath==='/workspace/board/',`/auth/callback/: final Member path ${finalPath}`);expect(!finalPath.includes('/degradation_club/'),'/auth/callback/: legacy prefix returned');expect(!pageErrors.length,`/auth/callback/ errors: ${pageErrors.join(' | ')}`);await c.close();
}

// Ordinary active Member: no public header, Board is default/first, identity owns profile.
{
  const c=await context('member'),p=await c.newPage(),pageErrors=[];p.on('pageerror',e=>pageErrors.push(e.message));
  await p.goto(base+'/workspace/',{waitUntil:'domcontentloaded'});try{await p.waitForURL(u=>new URL(u).pathname==='/workspace/board/',{timeout:5000})}catch{errors.push(`/workspace/: Member default did not resolve to Board; actual=${p.url()}`)}await workspaceShell(p,'/workspace/board/ member');
  expect(await p.getByRole('link',{name:'COMMUNITY BOARD'}).count()===1,'Member Workspace: Board link missing');
  expect(await p.getByRole('link',{name:'HOME'}).count()===0,'Member Workspace: HOME survived ordinary primary navigation');
  expect(await p.getByRole('link',{name:'МОЯ АКТИВНОСТЬ'}).count()===1,'Member Workspace: Activity link missing');
  expect(await p.locator('.dcw-session-profile--link').getAttribute('href')==='/workspace/#profile','Member Workspace: identity does not own profile entry');
  expect(await p.locator('.dc-global-header').count()===0,'Member Workspace: public header leaked into private shell');
  expect(!pageErrors.length,`/workspace/ member errors: ${pageErrors.join(' | ')}`);await c.close();
}

// First-entry Member: existing FIRST_ARTIFACT_REQUIRED state creates focus only; skip is session-only UI.
{
  const c=await context('member-first'),p=await c.newPage(),pageErrors=[];p.on('pageerror',e=>pageErrors.push(e.message));
  await p.goto(base+'/workspace/board/',{waitUntil:'domcontentloaded'});await workspaceShell(p,'/workspace/board/ first-entry');
  try{await p.locator('body.dc-board-first-entry-focus').waitFor({state:'attached',timeout:5000})}catch{errors.push('/workspace/board/: FIRST_ARTIFACT_REQUIRED spotlight did not activate')}
  try{await p.getByRole('button',{name:'Скрыть подсказку до следующего входа'}).waitFor({state:'visible',timeout:3000});await p.getByRole('button',{name:'Скрыть подсказку до следующего входа'}).click();await p.waitForFunction(()=>!document.body.classList.contains('dc-board-first-entry-focus'))}catch(e){errors.push(`/workspace/board/: first-entry skip failed: ${e.message}`)}
...
// Owner retains root Workspace navigation and can cross child surfaces without recovery login.
{
  ...
  await p.goto(base+'/workspace/',{waitUntil:'domcontentloaded'});await workspaceShell(p,'workspace navigation sequence');
  try{
    await p.getByRole('link',{name:'COMMUNITY BOARD'}).waitFor({state:'visible',timeout:4000});
    await p.getByRole('link',{name:'COMMUNITY BOARD'}).click();await p.waitForURL(u=>new URL(u).pathname==='/workspace/board/');
    await p.getByRole('link',{name:'HOME'}).click();await p.waitForURL(u=>new URL(u).pathname==='/workspace/'&&new URL(u).hash==='#home');await p.waitForFunction(()=>document.querySelector('#topTitle')?.textContent?.trim()==='HOME');
    await p.getByRole('link',{name:'MEMBERSHIP REVIEW'}).waitFor({state:'visible',timeout:4000});await p.getByRole('link',{name:'MEMBERSHIP REVIEW'}).click();await p.waitForURL(u=>new URL(u).pathname==='/workspace/review/');
    await p.getByRole('link',{name:'МОЯ АКТИВНОСТЬ'}).click();await p.waitForURL(u=>new URL(u).pathname==='/workspace/'&&new URL(u).hash==='#activity');await p.waitForFunction(()=>document.querySelector('#topTitle')?.textContent?.trim()==='MY ACTIVITY');
    await p.getByRole('link',{name:'COMMUNITY BOARD'}).click();await p.waitForURL(u=>new URL(u).pathname==='/workspace/board/');
    await p.getByRole('link',{name:'МОЙ КЛУБ'}).click();await p.waitForURL(u=>new URL(u).pathname==='/workspace/'&&new URL(u).hash==='#club');await p.waitForFunction(()=>document.querySelector('#topTitle')?.textContent?.trim()==='MY CLUB');expect((await p.locator('#appView').innerText()).includes('Мой клуб'),'Workspace sequence: My Club did not recover after child surfaces');
...
{
  const c=await context('member'),p=await c.newPage(),pageErrors=[];p.on('pageerror',e=>pageErrors.push(e.message));
  await p.goto(base+'/workspace/board/',{waitUntil:'domcontentloaded'});await workspaceShell(p,'/workspace/board/');
  ...
  expect(await p.locator('[data-workspace-sidebar]:visible').count()===1,'/workspace/board/: shell missing');expect(await p.locator('#boardFilters .dc-board-filter').count()>=5,'/workspace/board/: filters missing');expect(await p.locator('.dc-spatial-controls').count()===1,'/workspace/board/: pan/zoom controls missing');
