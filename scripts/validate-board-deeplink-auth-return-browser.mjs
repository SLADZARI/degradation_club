...
try{
  ...
  await artifact.goto(`${base}/__deeplink_harness__?from=share&focus=artifact:${ART}`);await artifact.waitForFunction(()=>globalThis.__DC_FOCUS?.type==='artifact');const artifactShare=artifact.locator('.dc-notice > [data-board-share]');await artifactShare.waitFor({state:'visible'});let state=await artifact.evaluate(()=>({focus:globalThis.__DC_FOCUS,hidden:document.querySelector('.dc-artifact-overlay').hidden,share:!!document.querySelector('.dc-notice > [data-board-share]'),legacyActionsHidden:getComputedStyle(document.querySelector('.dc-notice__actions')).display==='none'}));expect(state.focus?.id===ART&&state.hidden===false,'artifact deeplink: existing fullscreen open/focus request missing');expect(state.share&&state.legacyActionsHidden,'artifact share: control must remain visible outside hidden fullscreen actions');await artifactShare.click();const copied=await artifact.evaluate(()=>globalThis.__DC_COPIED);const copiedUrl=new URL(copied);expect(copiedUrl.origin===base&&copiedUrl.pathname==='/workspace/board/'&&copiedUrl.searchParams.get('from')==='share'&&copiedUrl.searchParams.get('focus')===`artifact:${ART}`,`artifact share: canonical URL mismatch ${copied}`);expect((await artifactShare.textContent())?.includes('СКОПИРОВАНО'),'artifact share: copied feedback missing');
  ...
  const mobile=await browser.newPage({viewport:{width:390,height:844}});await mobile.goto(`${base}/__deeplink_harness__`);const mobileShare=mobile.locator('.dc-notice > [data-board-share]');await mobileShare.waitFor({state:'visible'});const mobileBox=await mobileShare.boundingBox();expect(!!mobileBox&&mobileBox.width>0&&mobileBox.height>=26,'mobile share: compact visible control missing');await mobile.close();
...
console.log('✓ Artifact direct focus, visible card Share, canonical URL, safe close, Back and Forward');
console.log('✓ Entity deep-link keeps a visible direct-card Share without second detail renderer');
console.log('✓ compact Share remains visible at 390px mobile viewport');
