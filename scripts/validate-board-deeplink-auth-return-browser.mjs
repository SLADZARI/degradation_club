...
const mime={'.js':'text/javascript; charset=utf-8','.css':'text/css; charset=utf-8','.html':'text/html; charset=utf-8','.webp':'image/webp'};
...
try{
  ...
  expect(await board.locator('.dc-artifact-overlay').evaluate(el=>el.hidden),'sender: Share trigger opened Artifact overlay');
  ...
  expect(await board.locator('.dc-artifact-overlay').evaluate(el=>el.hidden),'sender: explicit copy opened Artifact overlay');
  await sender.getByRole('button',{name:'Закрыть'}).click();
  expect(await artifactShare.evaluate(el=>document.activeElement===el),'sender: close did not restore focus to Share trigger');
  await board.close();
