...
`;
function harness(){return `<!doctype html><html><body>
<div id="boardHost"><article class="dc-notice" data-artifact="${ART}"><h3>Artifact</h3><div class="dc-notice__actions"></div></article><article class="dc-projection" data-source-id="${ENT}"><h3>Entity</h3><div class="dc-notice__actions"><a href="/events/fuengirola/">ОТКРЫТЬ →</a></div></article></div>
...
try{
  ...
  await artifact.goto(`${base}/__deeplink_harness__?from=share&focus=artifact:${ART}`);await artifact.waitForFunction(()=>globalThis.__DC_FOCUS?.type==='artifact');let state=await artifact.evaluate(()=>({focus:globalThis.__DC_FOCUS,hidden:document.querySelector('.dc-artifact-overlay').hidden,share:!!document.querySelector('[data-board-share]')}));expect(state.focus?.id===ART&&state.hidden===false,'artifact deeplink: existing fullscreen open/focus request missing');expect(state.share,'artifact deeplink: share action missing');await artifact.locator('.dc-notice [data-board-share]').click();const copied=await artifact.evaluate(()=>globalThis.__DC_COPIED);const copiedUrl=new URL(copied);expect(copiedUrl.origin===base&&copiedUrl.pathname==='/workspace/board/'&&copiedUrl.searchParams.get('from')==='share'&&copiedUrl.searchParams.get('focus')===`artifact:${ART}`,`artifact share: canonical URL mismatch ${copied}`);
  ...
  const entity=await browser.newPage();await entity.goto(`${base}/__deeplink_harness__?focus=entity:${ENT}`);await entity.waitForFunction(()=>globalThis.__DC_FOCUS?.type==='entity');state=await entity.evaluate(()=>({focus:globalThis.__DC_FOCUS,hidden:document.querySelector('.dc-artifact-overlay').hidden,share:!!document.querySelector('.dc-projection [data-board-share]')}));expect(state.focus?.id===ENT&&state.focus?.open===false,'entity deeplink: projection focus request incorrect');expect(state.hidden===true,'entity deeplink: must not open Artifact overlay');expect(state.share,'entity deeplink: share action missing');await entity.close();
...
console.log('✓ Artifact direct focus, canonical Share URL, safe close, Back and Forward');
console.log('✓ Entity deep-link focuses projection without second detail renderer');
