...
const session={user:{id:'${USER}',email:'member@example.test'}};const artifact=${JSON.stringify(artifact)};const profile=${JSON.stringify(profile)};
function rowsFor(table,state){if(table==='dc_artifacts'){if(state.eq.status==='draft')return [];return [artifact]}if(table==='profiles')return [profile];if(table==='dc_artifact_board_positions')return [{artifact_id:artifact.id,x:6000,y:4000,rotation:.35,size_class:'S',position_version:1}];return []}
...
for(const [engine,browserType] of [['chromium',chromium],['webkit',webkit]]){
  ...
  try{
    for(const viewport of [{width:1280,height:800},{width:390,height:844}]){
      ...
      await page.addInitScript(()=>{globalThis.__TRACE=[];globalThis.__DC_COPIED='';Object.defineProperty(navigator,'clipboard',{configurable:true,value:{writeText:async value=>{globalThis.__DC_COPIED=value}}});document.execCommand=()=>true;for(const type of ['pointerdown','pointerup','click'])document.addEventListener(type,event=>{globalThis.__TRACE.push({type,target:event.target?.tagName||'',className:String(event.target?.className||''),share:!!event.target?.closest?.('[data-board-share]'),x:event.clientX,y:event.clientY})},true)});
      await page.goto(`${base}/workspace/board/?debug=must-not-leak`);
      const card=page.locator(`.dc-notice[data-artifact="${ART}"]`);await card.waitFor({state:'visible'});await page.waitForFunction(id=>document.querySelector(`.dc-notice[data-artifact="${id}"]`)?.classList.contains('is-own-movable'),ART);const share=card.locator(':scope > [data-board-share]');await share.waitFor({state:'visible'});
      ...
      const box=await share.boundingBox();expect(!!box,`${engine}/${viewport.width}: Share has no bounding box`);if(!box){await context.close();continue}const point={x:box.x+box.width/2,y:box.y+box.height/2};const hit=await page.evaluate(({x,y})=>{const el=document.elementFromPoint(x,y);return {tag:el?.tagName||'',className:String(el?.className||''),isShare:!!el?.closest?.('[data-board-share]')}},point);expect(hit.isShare,`${engine}/${viewport.width}: FULL STACK center hit target is ${hit.tag}.${hit.className}, not Share`);
      ...
      expect(!state.copied,`${engine}/${viewport.width}: Share copied before explicit postcard action`);expect(state.overlayHidden,`${engine}/${viewport.width}: FULL STACK Share opened Artifact; hit=${JSON.stringify(hit)} trace=${JSON.stringify(state.trace)}`);expect(!state.dragging,`${engine}/${viewport.width}: FULL STACK Share started drag; trace=${JSON.stringify(state.trace)}`);
      ...
      await postcard.getByRole('button',{name:'КОПИРОВАТЬ ССЫЛКУ'}).click();await page.waitForTimeout(80);state=await page.evaluate(()=>({copied:globalThis.__DC_COPIED||'',overlayHidden:document.querySelector('.dc-artifact-overlay')?.hidden??true,dragging:document.documentElement.dataset.boardDragging||''}));expect(state.copied===shown,`${engine}/${viewport.width}: explicit postcard copy mismatch`);expect(state.overlayHidden,`${engine}/${viewport.width}: postcard copy opened Artifact`);expect(!state.dragging,`${engine}/${viewport.width}: postcard copy started drag`);expect(await postcard.getByText('СКОПИРОВАНО').count()===1,`${engine}/${viewport.width}: postcard copy feedback missing`);
...
console.log('✓ elementFromPoint resolves visible Share across Chromium/WebKit desktop/mobile');
console.log('✓ one physical Share click opens only postcard, never fullscreen or drag');
console.log('✓ explicit postcard copy emits clean dedicated Artifact share URL');
