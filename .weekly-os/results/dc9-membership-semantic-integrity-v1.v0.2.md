...
# MP | Dementor Club | BUILD | DC-9 / Membership Semantic Integrity v1 | v0.2
...
## Governance correction from v0.1
v0.1 used the noncanonical gate label `G3_BUILD` and artifact status `ACTIVE`. v0.2 corrects only MP_DSL governance vocabulary:
- gate → `G5_BUILD`;
- artifact status → `DRAFT`;
- operational body status remains `IN_PROGRESS`.

This correction does **not** change Product, Domain, Architecture, Design, Membership lifecycle, QA scope, implementation code, database state, production merge authorization or deploy authorization.
...
## Release boundary
Implementation may create a migration file in the implementation branch/PR. That does **not** authorize applying it to live Supabase.
