...
Status: **INTEGRATED WORKING CANON / source map**  
Updated: **2026-09-16**

## Purpose

This file is the canonical navigation layer for the Dementor Club Product & Marketing package.

It does not replace the authorities below. It records which source owns which question.

Core rules:

**One question → one primary authority.**

**PRIMARY AUTHORITY DEFINES MEANING. CONSOLIDATION DOES NOT SILENTLY REDEFINE.**

**sourceSystem ≠ authorityType.**

Individual source documents retain their own internal status labels and scope. Integration into this index does not silently promote unrelated DRAFT / REFERENCE material.
...
Rule: **DISTRIBUTE THE THING, NOT THE ORG CHART.**
...
Rule: **PAYMENT FOLLOWS VALUE, NOT STATUS OR POWER.**
...
# Product → Production bridge

Phase 0 factual authority:
- `operations/PRODUCT_TO_PRODUCTION_AUDIT_2026-09-15.md`
- `operations/PHASE_0_TRUTH_REALITY_ALIGNMENT_2026-09-15.md`
- `operations/PHASE_0_ADDENDUM_RELEASE_ACCESS_DECISIONS_2026-09-15.md`

Phase 1 semantic implementation contract:
- `operations/PHASE_1_CURRENT_PROGRAM_THING_PROJECTION_CONTRACT_V1.md`
- `operations/PHASE_1A_CURRENT_PROGRAM_COMPOSITION_V0.md`

Released implementation evidence remains owned by Result/G8 artifacts, not by this index.

---

# Current sequence after Phase 1

1. Resolve explicit source conflicts before amplification.
2. Prove one complete `THING → EXPERIENCE → COMPLETION → CONTINUATION → RETURN` loop.
3. Instrument Product Health only where a concrete question exists.
4. Extend the proven pattern to other Things without copying accidental UI.
5. Contribution / Intervention / Monetization remain optional downstream modes, not mandatory depth.

First Phase 2 proving case:
**`Думай с опасностью` — PUBLIC RELEASE → completion → certificate → one honest next Thing → real reason to return.**
