...
if(cfg?.enabled&&cfg.url&&cfg.publishableKey){
  ...
  const announceUser=user=>{
    window.DEMENTOR_AUTH_USER=user||null;
    window.dispatchEvent(new CustomEvent('dc-auth-ready',{detail:{signedIn:Boolean(user),email:user?.email||null}}));
  };
  ...
  announceUser(session?.user||null);
  client.auth.onAuthStateChange((_event,next)=>announceUser(next?.user||null));
