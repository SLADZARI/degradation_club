...
const expectedTargetContact=beforeTarget.contact+(first.metricDeltas?.target?.contact||0)+(first.eventImpact?.metrics?.contact||0);
assert.ok(Math.abs(encounter.actors.B.state.contact-expectedTargetContact)<1e-9,'reaction + impulse + memory semantics + event impact must all reach target state');
