...
await page.locator('#to-setup').click();assert.equal(await page.locator('#top-status').textContent(),'SETUP');await page.locator('[data-mode="step"]').click();await page.locator('#play').click();assert.equal(await page.locator('#top-status').textContent(),'TALK');
assert.equal(await page.locator('#actor-a').getAttribute('data-character'),'character-02','selected player rig persists into TALK');assert.equal(await page.locator('#actor-b').getAttribute('data-character'),'character-02','opponent uses female character rig');
...
await page.locator('#rerun').click();assert.equal(await page.locator('#top-status').textContent(),'BRAIN');assert.equal(await page.locator('#replay-note').isVisible(),true);assert.equal(await page.locator('#brain-editor input:disabled').count(),1,'counterfactual replay locks the non-target control');
