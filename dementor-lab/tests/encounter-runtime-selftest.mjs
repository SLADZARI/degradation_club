...
const changed=applyHotPatch(h,{kind:'reduce-repeat',actorId:'A',nodeId:repeatId});assert.equal(changed.before,4);assert.equal(changed.after,1,'HOT PATCH breaks the dangerous loop instead of shaving one repetition');assert.equal(h.turn,beforeTurn);assert.equal(JSON.stringify(h.actors.A.state.memory),beforeMemory);assert.equal(h.transcript.length,beforeTranscript);assert.equal(h.status,'NEXT_TURN');const afterPatchTurn=executeActorTurn(h);assert.equal(afterPatchTurn.trace.selectedReaction,'explain');assert.equal(h.pendingRepeats.A,null,'loop-break patch prevents new pending repeats on the repaired route');
