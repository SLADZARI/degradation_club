...
function pauseEdge(actor,visited=[]){
  if(visited.map(id=>node(actor,id)).some(n=>n?.type==='pause'))return null;
  ...
  return actor?.brainGraph?.edges?.find(e=>REACTION_TYPES.has(node(actor,e.to)?.type))||null;
}
...
export function recommendOneCausePatch(encounter,breakpoint=null){
  ...
  if(edge){
    return {
      kind:'insert-pause',actorId:'A',edgeId:edge.id,nodeId:`hot-pause-${edge.id}`,
      title:'ВСТАВИТЬ ПАУЗУ',
      description:'ПЕРЕД ОТВЕТОМ → ПАУЗА',
      before:edge.id,after:`hot-pause-${edge.id}`
    };
  }
...
export function counterfactualMutationFromRecommendation(rec){
  ...
  if(rec.kind==='insert-pause')return {kind:'insert-pause',edgeId:rec.edgeId,nodeId:rec.nodeId};
