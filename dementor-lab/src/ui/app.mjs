...
function nodeSubtitle(n){const map={criticism:'что произошло',ignore:'что не заметил',resentment:'что накопилось',trust:'что сохранилось',beright:'доказать своё',beliked:'сохранить одобрение',understand:'разобраться',explain:'объяснить',agree:'согласиться',joke:'пошутить',silent:'промолчать',pressure:'давить',repeat:'повторить',stop:'остановиться',ifbrain:'если мозг нагрет',pause:'пауза',interrupt:'перехват'};return map[n.type]||familyLabel(n.type)}
