...
const graph=(id,nodes,edges)=>({id,nodes,edges});

export const BRAIN_PRESETS=Object.freeze([
  {
    id:'always-right',
    label:'Я ВСЕГДА ПРАВ',
    graph:graph('preset-always-right',[
      node('p1-trigger','criticism',{},160,30),
      node('p1-state','resentment',{},160,150),
      node('p1-impulse','beright',{weight:5},160,270),
      node('p1-reaction','explain',{},160,390),
      node('p1-repeat','repeat',{count:3},160,510)
    ],[
  ...
  },
  {
    ...
    label:'СЕЙЧАС ВСЁ ОБЪЯСНЮ',
    graph:graph('preset-explain-everything',[
      node('p2-trigger','criticism',{},160,30),
      node('p2-impulse','understand',{weight:2},70,170),
      node('p2-right','beright',{weight:4},250,170),
      node('p2-reaction','explain',{},160,330),
      node('p2-repeat','repeat',{count:4},160,470)
    ],[
  ...
  },
  {
    ...
    graph:graph('preset-keep-peace',[
      node('p3-trigger','criticism',{},160,30),
      node('p3-trust','trust',{delta:1},160,150),
      node('p3-liked','beliked',{weight:5},160,270),
      node('p3-agree','agree',{},80,400),
      node('p3-silent','silent',{},250,400),
      node('p3-stop','stop',{},160,540)
    ],[
  ...
  {
    id:'just-asked',
    label:'Я ПРОСТО СПРОСИЛ',
    graph:graph('preset-just-asked',[
  ...
  {
    id:'see-what-happens',
    label:'ПОСМОТРИМ, ЧТО БУДЕТ',
    graph:graph('preset-see-what-happens',[
