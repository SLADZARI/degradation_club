export function visualStateFromMetrics(s){
  ...
  return {
    eyes,brows,mouth,
    motion:{
      amplitude:energy<=25?.45:energy<=60?.72:1,
      headDrop:energy<=25?1:0,
      headInstability:brain>=85?Math.min(1,(brain-84)/16):0,
      gestureSharpness:tension>=55?Math.min(1,(tension-54)/46):0,
      orientToPartner:contact>=75?.5:contact<=25?-.35:0
    }
  };
}
...
export class CharacterRenderer{
  ...
  el(name){
    ...
    return this.svg.querySelector('#'+CSS.escape(`${this.side}-${name}`));
  }
  variants(group,active){
    const groups={
      eyes:['neutral','tense','sleepy','overheat'],
      brows:['neutral','tense','angry'],
      mouth:['neutral','soft','tense','open']
    };
  ...
  render(character){
    ...
    this.variants('mouth',face.mouth||'neutral');

    const head=this.el('head-rig');
    if(head){
      head.style.transformOrigin='352px 270px';
      const sign=this.side==='A'?1:-1;
      head.style.transform=`translateY(${(m.headDrop||0)*6}px) rotate(${((m.headInstability||0)+(m.orientToPartner||0))*2*sign}deg)`;
    }
    const left=this.el('body-arm-left'),right=this.el('body-arm-right');
    const amp=m.amplitude??1,sharp=m.gestureSharpness||0;
  ...
  breakdown(character,reason){
    ...
    this.root?.classList.remove('lost','brain','energy','contact');
    this.root?.classList.add('lost');
    const key=String(reason||'').toUpperCase().includes('ENERGY')?'energy':
      String(reason||'').toUpperCase().includes('CONTACT')?'contact':'brain';
